...
data Expr = Var Id --variable -- x
          | Lam Id Expr --lambda expression i.e \x -> e
          | Expr :@: Expr --applicaiton of function to argument i.e. d e
          | Let Id Expr Expr --let expression let  x = d in e
...
eval (Let x d e) r = let v = eval d ((x,v):r) in eval e ((x, v):r)
